resource "null_resource" "update_kubeconfig" {
  ...
  depends_on = [module.eks.cluster_id, time_sleep.wait_for_cluster]
}
...
resource "null_resource" "crdapply" {
  ...
  depends_on = [null_resource.namespaces, null_resource.update_kubeconfig]
}
...
resource "helm_release" "certmanager" {
  ...
  timeout    = 600
  wait       = true
  depends_on = [null_resource.crdapply, null_resource.update_kubeconfig, module.eks.cluster_id, time_sleep.wait_for_cluster]
}
